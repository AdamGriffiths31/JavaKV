...
class InMemoryKeyValueStoreTest {
  ...
  @Test
  @DisplayName("Should throw exception for empty key in delete")
  ...
  @Test
  @DisplayName("Should throw exception for empty key in exists")
  void testExistsEmptyKey() {
  ...
  @Test
  @DisplayName("Should throw StorageException when storage engine fails on clear")
  void testClearStorageFailure() {
    testStorage = new TestStorageEngine();
    store = new InMemoryKeyValueStore(testStorage);

    // Put some values successfully
    store.put("key1", "value1");
    store.put("key2", "value2");
    ...
    assertThatThrownBy(() -> store.clear())
      .isInstanceOf(StorageException.class)
      .hasMessageContaining("Failed to persist CLEAR operation");

    // Verify the in-memory store was NOT updated (keys still exist)
    assertThat(store.exists("key1")).isTrue();
    assertThat(store.exists("key2")).isTrue();
  }
